...
from networks3 import ActorNetwork, CriticNetwork, ValueNetwork
import config
...
class SAC_agent(object):
    def __init__(self, input_dims, n_actions, max_action):
        ...
        # a = T.tensor([[0.13,0.13,0.1,0.2,0.3,0.4]]).to(self.actor.device)
        # writer.add_graph('net/actor',self.actor,(a,))
        ...
        # a = T.tensor([[0.13,0.13,0.1,0.2,0.3,0.4]]).to(self.actor.device)
        # b = T.tensor([[0.13, 0.13]]).to(self.actor.device)
        # writer.add_graph('net/critic',self.critic_2,(a,b))

        self.critic_1_target = CriticNetwork(C_LR, input_dims, n_actions, hidden_sizes, name='critic_1_target')
        # a = T.tensor([[0.13,0.13,0.1,0.2,0.3,0.4,0.2,0.3]]).to(self.actor.device)
        # b = T.tensor([[0.13, 0.13]]).to(self.actor.device)
        # writer.add_graph('net/critic_target',self.critic_1_target,(a,b))
    ...
    def learn(self):
        ...
        loss_pi = (self.scale * log_prob2 - q_pi).mean()  # Entropy-regularized policy loss
        ...
        # writer.add_graph( self.SAC_agent(),)
        # writer.close()
