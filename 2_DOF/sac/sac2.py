...
class SAC_agent(object):
    def __init__(self, input_dims, n_actions, max_action):
        ...
        self.value = ValueNetwork(C_LR, input_dims,hidden_sizes, name='value')
        self.target_value = ValueNetwork(C_LR, input_dims,hidden_sizes, name='target_value')
    ...
    ##----------------------------更新value-----------------------------------------
        ...
        #計算這個state的價值/shape torch.Size([32])//[without view(-1)] => torch.Size([32, 1])
        ...
        actions, log_probs = self.actor.sample_normal(state, reparameterize=config.reparameterize_critic)
        log_probs = log_probs.view(-1)
        q1_new_policy = self.critic_1.forward(state, action)
        q2_new_policy = self.critic_2.forward(state, action)
        critic_value = T.min(q1_new_policy, q2_new_policy) #torch.Size([32, 1])
        critic_value = critic_value.view(-1) #torch.Size([32])

        self.value.optimizer.zero_grad()  # 將梯度初始化為0
        # value networks loss and backprop (V=Q-log)
        # value_approx = critic_value - log_probs  #(版1)
        # print('log_probs',log_probs.shape)
        value_approx = critic_value - self.scale * log_probs  #(版2)
        #.detach() 返回一个新的Variable，从当前计算图中分离下来的，但是仍指向原变量的存放位置,不同之处只是requires_grad为false，得到的这个Variable永远不需要计算其梯度，不具有grad。
        value_loss = F.mse_loss(value, value_approx.detach())
        value_loss.backward(retain_graph=True) #反向傳播,create_graph参数的作用是，如果为True可計算高階微分
        self.value.optimizer.step() #更新優化器學習率

        ##----------------------------更新policy-----------------------------------------

        actions, log_probs = self.actor.sample_normal(state, reparameterize=config.reparameterize_actor)
        log_probs = log_probs.view(-1)
        q1_new_policy = self.critic_1.forward(state, actions)
        q2_new_policy = self.critic_2.forward(state, actions)
        critic_value = T.min(q1_new_policy, q2_new_policy)
        critic_value = critic_value.view(-1)
        # actor_loss=(log_probs-critic_value).mean() #(版1)
        actor_loss = (self.scale * log_probs - critic_value).mean()  #(版2)
       ...
       ##----------------------------更新 Q ------------------------------------------
        ...
        # q_hat=self.scale*reward+gamma*value_  #Q_=aR+r*v_(版1)
        q_hat = reward + gamma * value_  #(版2)
