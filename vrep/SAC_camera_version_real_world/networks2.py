...
class CriticNetwork(nn.Module):
    ...
    def forward(self, state, action):
        ...
        #-------其他特徵-------
        # other_feature = state[:,1:2,:,:]
        # other_feature =other_feature.view(-1,64*64)  #攤平
        # other_feature = other_feature[...,:other_feature_dim]
        # other_feature = other_feature.view(-1,other_feature_dim)
    ...
    def load_checkpoint(self, path):
        checkpoint_file = config.MODEL_PATH+'/model/'+ path[0] + '/net/' + path[1] + '/' + self.name + '_sac'
...
class ValueNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        img = img.view(-1, 1024)
        img = self.linear(img)
        # feature = T.cat([img, other_feature], dim=1)
        feature = img
